...
class nnModel(object):
    ...
    def _load_model(self, url, args):
        ...
        if isinstance(checkpoint, tuple):
            # 格式1: (model_state_dict, ob_rms)
            if len(checkpoint) == 2:
                model_pretrained, ob_rms = checkpoint
            else:
                # 格式2: 只有模型，没有 ob_rms
                model_pretrained = checkpoint[0]
                ob_rms = None
        elif isinstance(checkpoint, dict):
            # 检查是否有 'model_state_dict' 键（新训练格式）
            if 'model_state_dict' in checkpoint:
                model_pretrained = checkpoint['model_state_dict']
                ob_rms = checkpoint.get('ob_rms', None)
            # 检查是否有 'model' 键
            elif 'model' in checkpoint:
                model_pretrained = checkpoint['model']
                ob_rms = checkpoint.get('ob_rms', None)
            else:
                # 假设整个字典就是模型状态
                model_pretrained = checkpoint
                ob_rms = None
        else:
            # 格式4: 直接是模型状态字典
            model_pretrained = checkpoint
        ...
        for k, v in load_dict.items():
            # 只处理tensor类型的值
            if isinstance(v, torch.Tensor) and len(v.size()) <= 3:
