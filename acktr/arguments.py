...
def get_args():
    ...
    parser.add_argument(
       '--container_size', default=(10, 10, 10), type=int, nargs=3, help='container size along x, y and z axis'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--item-size-range', default=(2,2,2,5,5,5), type=int, nargs=6, help='the item size range, (min_width, min_length, min_height, max_width, max_length, max_height)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--save_interval', default=10, type=int,  help='save interval, one save per n updates (default: 10)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--stat_decay', default=0.99, type=float,  help='KFAC running statistics decay rate (default: 0.99)'
    )
    parser.add_argument(
        '--kl_clip', default=0.001, type=float,  help='KFAC KL divergence clipping threshold (default: 0.001)'
    )
    parser.add_argument(
        '--damping', default=1e-2, type=float,  help='KFAC damping factor for numerical stability (default: 1e-2)'
    )
    parser.add_argument(
    ...
    # Reliability features arguments
    parser.add_argument(
        '--uncertainty-enabled', action='store_true', default=False,
        help='Enable placement noise during training to simulate real-world uncertainty'
    )
    parser.add_argument(
        '--uncertainty-std-x', default=0.5, type=float,
        help='Standard deviation for placement noise in x direction (default: 0.5)'
    )
    parser.add_argument(
        '--uncertainty-std-y', default=0.5, type=float,
        help='Standard deviation for placement noise in y direction (default: 0.5)'
    )
    parser.add_argument(
        '--uncertainty-std-z', default=0.1, type=float,
        help='Standard deviation for placement noise in z direction (default: 0.1)'
    )
    parser.add_argument(
        '--visual-feedback-enabled', action='store_true', default=False,
        help='Enable visual feedback module for height map updates'
    )
    parser.add_argument(
        '--parallel-motion-enabled', action='store_true', default=False,
        help='Enable parallel entry motion primitive with buffer space'
    )
    parser.add_argument(
        '--buffer-range-x', default=1, type=int,
        help='Buffer space range in x direction (default: 1)'
    )
    parser.add_argument(
        '--buffer-range-y', default=1, type=int,
        help='Buffer space range in y direction (default: 1)'
    )
    parser.add_argument(
        '--camera-config', default=None, type=str,
        help='Path to camera configuration file for visual feedback'
    )
    args = parser.parse_args()

    # Convert lists to tuples for container_size and item_size_range
    if isinstance(args.container_size, list):
        args.container_size = tuple(args.container_size)
    if isinstance(args.item_size_range, list):
    ...
    # Reliability features configuration
